...
$S(t_2)$ and $S(t_1)$, where $t_2 > t_1$. Defining $V_s(t)=V(0, t, t)$ and 
$I_s(t)=I(0, t, t)$:

\begin{equation}
    S(t_2) = S(t_1) \frac{F(0, t_2)}{F(0, t_1)} e^{- \frac{1}{2} (V_s(t_2) - 
    V_s(t_1)) + I_s(t_2) - I_s(t_1)}
\end{equation}

Focussing on the stochastic term $I_s(t_2) - I_s(t_1)$:

\begin{equation}
    I_s(t_2) - I_s(t_1) = \sum_{i=1}^n \biggl( \sigma_i(t_2) 
    \int_0^{t_2} e^{-\alpha_i(t_2-u)} dz_i(u) - \sigma_i(t_1) 
    \int_0^{t_1} e^{-\alpha_i(t_1-u)} dz_i(u) \biggr)
\end{equation}
...
\begin{equation}
    I_s(t_2) - I_s(t_1) = \sum_{i=1}^n \biggl( \sigma_i(t_2) 
    F_i(t_2) - \sigma_i(t_1) F_i(t_1) \biggr)
\end{equation}


When simulating, this expression will be adaped to the filtration $\mathcal{F}_{t_1}$, hence the
$F_i(t_1)$ will have been realised. $F_i(t_2)$ will be part realised, hence it is instructive
to split this into it's deterministic and stochastic parts, as of $t_1$. First making the
substituting $t_2 = t_1 + t_2 - t_1$:

\begin{eqnarray}
    \nonumber
    F_i(t_2) = e^{-\alpha_i(t_2 - t_1)} \int_0^{t_2} e^{-\alpha_i(t_1-u)} dz_i(u) \\
        = e^{-\alpha_i(t_2 - t_1)} \biggl(\int_0^{t_1} e^{-\alpha_i(t_1-u)} dz_i(u) +
        \int_{t_1}^{t_2} e^{-\alpha_i(t_1-u)} dz_i(u) \biggr)
\end{eqnarray}

Noticing that the first integral is the definition of $F_i(t_1)$, the function $F_i$ can
be defined recursively:

\begin{equation}
    F_i(t_2) = e^{-\alpha_i(t_2 - t_1)}F_i(t_1) + \int_{t_1}^{t_2} e^{-\alpha_i(t_2-u)} dz_i(u)
\end{equation}
...
and mean reversion parameter $\alpha_i$.
